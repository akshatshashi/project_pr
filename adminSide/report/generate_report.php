...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 10, "SNU FOODs", 0, 1, 'C');
...
$pdf->ChapterBody("Total Revenue Today: RS " . number_format($totalRevenueToday, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Week: RS " . number_format($totalRevenueThisWeek, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Month: RS " . number_format($totalRevenueThisMonth, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Year: RS " . number_format($totalRevenueThisYear, 2));
$pdf->Ln();
...
$header = array('Date','Week' , 'Revenue (RS)');
$data = array();
...
$header = array('Date','Month' , 'Revenue (RS)');
$data = array();
...
$header = array('Date','Day' , 'Item Category', 'Revenue (RS)');
$data = array();
while ($row = mysqli_fetch_assoc($dailyCategoryRevenue)) {
...
$header = array('Date','Week' , 'Item Category', 'Revenue (RS)');
$data = array();
while ($row = mysqli_fetch_assoc($weeklyCategoryRevenue)) {
...
$header = array('Date','Month' , 'Item Category', 'Revenue (RS)');
$data = array();
while ($row = mysqli_fetch_assoc($monthlyCategoryRevenue)) {
...
$header = array('Date', 'Item Category', 'Revenue (RS)');
$data = array();
